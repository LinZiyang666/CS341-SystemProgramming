...
void run_server(char *port)
{
    ...
    while (!endSession)
    { // while the server has not closed (endSession != 0) => giant while-loop in which we try to accept clients' connections

        int client_fd = accept(sock_fd, NULL, NULL);
        if (-1 == client_fd)
        {
            puts("I was stuck here\n");
            if (errno == EINTR)
            {
                // accept() was interrupted by our signal. Break out of the loop.
                break;
            }
            perror(NULL);
            exit(EXIT_FAILURE);
        ...
        { // shut down new client, continue accepting
